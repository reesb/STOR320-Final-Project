...
```{r}
ggplot(news2,aes(x = year, y = numwords, fill = year)) + geom_boxplot(outlier.shape = NA)  + theme_bw()
```
...
### By Month
```{r}
ggplot(news2,aes(x = month, y = numwords, fill = month)) + geom_boxplot(outlier.shape = NA)  + theme_bw()
```

### By Day of Week
```{r}
ggplot(news2,aes(x = day_of_week, y = numwords, fill = day_of_week)) + geom_boxplot(outlier.shape = NA)  + theme_bw() + ylim(0, 2500)
```
...
ggplot(AllTheNews2,aes(x = publication, y = subjectivity, fill = publication),outline=FALSE) + geom_boxplot(outlier.shape = NA)  + theme(axis.text.x = element_text(angle = 60, hjust = 1))
```

```{r}
ggplot(AllTheNews2,aes(x = publication, y = polarity, fill = publication),outline=FALSE) + geom_boxplot(outlier.shape = NA)  + theme(axis.text.x = element_text(angle = 60, hjust = 1))
```
...
```
```{r}
ggplot(news,aes(x = month, y = polarity, fill = month),outline=FALSE) + geom_boxplot(outlier.shape = NA)  + theme(axis.text.x = element_text(angle = 60, hjust = 1))
```

```{r}
AllTheNewsQuant <- select(AllTheNews2, -title, -text, -publication, -author, -date, -url)
