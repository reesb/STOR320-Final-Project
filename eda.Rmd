...
AllTheNews <- select(AllTheNews, title, date, year, month, day, day_of_week, url, textfixed, subjectivity, polarity)
AllTheNews <- AllTheNews %>% rename(text=textfixed)
...
AllTheNews <- AllTheNews %>% sample_n(10000)
OnlineNewsPopularity <- OnlineNewsPopularity %>% sample_n(10000)
NewsAggregatorDataset <- NewsAggregatorDataset %>% sample_n(10000)
...
news <- filter(news, (news$numwords >= 5) & (news$numwords <= 5000))
```
